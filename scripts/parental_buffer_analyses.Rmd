...
#Packages
  packages <- c('reshape','psych','rstatix','tidyverse', 'Rmisc', 'lme4','ggpubr','report')
...
#b) plot
  ...
  #try with lmer
  model_like_int <-lmer(data= DBehav_Longdif, LikeRESP~version*time+SEX+MRI_AGE+(1|Subject))
  model_like_noint <-lmer(data= DBehav_Longdif, LikeRESP~version+time+SEX+MRI_AGE+(1|Subject))
  anova(model_like_int,model_like_noint)
  summary(model_like_int)
...
#investigate with lmer and age and sex
  lmer_NervousRESP <-lmer(data= DBehav_Longdif, NervousRESP~version*time+MRI_AGE+SEX+(1|Subject))
...
DROI_50thr_cmamyg_R_wide <- reshape(DROI_50thr_cmamyg_R, timevar = "Parent_Alone",idvar="Subject",direction="wide")

varnames_cmamyg_r <- c(which(colnames(DROI_50thr_cmamyg_R)=="Subject"),
                       which(colnames(DROI_50thr_cmamyg_R)=="Parent_Alone"),
                       which(colnames(DROI_50thr_cmamyg_R)=="cmamyg_r_activation.noise_baseline"),
                       which(colnames(DROI_50thr_cmamyg_R)=="cmamyg_r_activation.plus_minus"))

                       
DBehav_Longdif <- merge(DBehav_Longdif,DROI_50thr_cmamyg_R[,varnames_cmamyg_r], by=c("Subject","Parent_Alone"),all.x=T, all.y=F)


DBehav_Wide <- merge(DBehav_Wide,
                     DROI_50thr_cmamyg_R_wide[,c("Subject",
                                                 "cmamyg_r_activation.noise_baseline.Alone",
                                                 "cmamyg_r_activation.noise_baseline.Parent",
                                                 "cmamyg_r_activation.plus_minus.Alone",
                                                 "cmamyg_r_activation.plus_minus.Parent")], by ="Subject", all.x=T, all.y=F)

cor.test(DBehav_Wide$Alone_Post_PlusMinMinus,DBehav_Wide$cmamyg_r_activation.noise_baseline.Alone)
cor.test(DBehav_Wide$Alone_Post_PlusMinMinus.ner,DBehav_Wide$cmamyg_r_activation.noise_baseline.Parent)
cor.test(DBehav_Wide$Parent_Post_PlusMinMinus,DBehav_Wide$cmamyg_r_activation.plus_minus.Alone)
cor.test(DBehav_Wide$Parent_Post_PlusMinMinus.ner,DBehav_Wide$cmamyg_r_activation.plus_minus.Parent)

summary(lmer(data=DBehav_Longdif, LikeRESP ~ Parent_Alone*time*cmamyg_r_activation.noise_baseline+(1|Subject)))
```
...
lmer_mPFC_noise_baseline<-lmer(data= DROI_mPFC, mPFC_activation.noise_baseline~Parent_Alone+(1|Subject))
...
#analysis with no confounding variables
lmer_mPFC_plus_minus<-lmer(data= DROI_mPFC, mPFC_activation.plus_minus~Parent_Alone+(1|Subject))
...
#analysis with no confounding variables
lmer_amyg_r_plus_minus<-lmer(data= DROI_amyg_r, amyg_r_activation.plus_minus~Parent_Alone+(1|Subject))
...
#examine separately for males and females
lmer_amyg_r_plus_minus_males <-lmer(data= DROI_amyg_r[DROI_amyg_r$SEX=="m",], amyg_r_activation.plus_minus~Parent_Alone + (1|Subject))
report(lmer_amyg_r_plus_minus_males)

lmer_amyg_r_plus_minus_females <-lmer(data= DROI_amyg_r[DROI_amyg_r$SEX=="f",], amyg_r_activation.plus_minus~Parent_Alone + (1|Subject))
...
#examine separately for males and females
lmer_amyg_l_plus_minus_males <-lmer(data= DROI_amyg_l[DROI_amyg_l$SEX=="m",], amyg_l_activation.plus_minus~Parent_Alone + (1|Subject))
report(lmer_amyg_l_plus_minus_males)

lmer_amyg_l_plus_minus_females <-lmer(data= DROI_amyg_l[DROI_amyg_l$SEX=="f",], amyg_l_activation.plus_minus~Parent_Alone + (1|Subject))
