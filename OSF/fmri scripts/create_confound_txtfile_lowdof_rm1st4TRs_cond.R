...
# It creates regressors separately for each task (cond1, cond2). 
# It removes the first 4 TRs from the file (dummy TRs, specific to ELFK study)
...
df_clean <- df %>%
  ...
  select(# average WM and CSF signals in derived masks
         ...
         rot_z,
         rot_z_derivative1,
         # each motion outlier (FD > 0.9 for task-based activation)
...
# remove coloumns of bad TR condound regressors that reflect the first 4 dummy TRs    
which_remove <- c()
