...
#multilevel linear regression controlling for sex and age
  lmer_mPFC_plus_minus_sex_age <- lmer(data=DROI_mPFC, 
                                       mPFC_activation.plus_minus~
                                         Parent_Alone_num*SEX_num_c + Parent_Alone_num_c*MRI_AGE_c +(1|Subject))
...
#multilevel linear regression controlling for sex, age and order but no interactions
  lmer_mPFC_plus_minus_sex_age_order <- lmer(data=DROI_mPFC, 
                                       mPFC_activation.plus_minus~
                                         Parent_Alone +SEX + order +(1|Subject))
  report(lmer_mPFC_plus_minus_sex_age_order)
  
#multilevel linear regression with no covariates
  lmer_mPFC_plus_minus<-lmer(data= DROI_mPFC, mPFC_activation.plus_minus~Parent_Alone_num+(1|Subject))
...
#multilevel linear regression controlling for sex age and order- categorical variables
  lmer_amyg_noise_baseline_sex_age_order <- lmer(data=DROI_amyg, 
                                           amyg_activation.noise_baseline~ Parent_Alone*SEX + Parent_Alone*MRI_AGE_c + Parent_Alone*order +(1|Subject))
 summary(lmer_amyg_noise_baseline_sex_age_order)

#multilevel linear regression controlling for sex and age
  lmer_amyg_noise_baseline_sex_age <- lmer(data=DROI_amyg, 
                                           amyg_activation.noise_baseline~ Parent_Alone_num*SEX_num + Parent_Alone_num*MRI_AGE_c +(1|Subject))
  report(lmer_amyg_noise_baseline_sex_age)
  
#multilevel linear regression controlling for sex, age, order but no interactions
  lmer_amyg_noise_baseline_sex_age_order_noint <- lmer(data=DROI_amyg, 
                                           amyg_activation.noise_baseline~ Parent_Alone +SEX + order+ MRI_AGE_c +(1|Subject))
  report(lmer_amyg_noise_baseline_sex_age_order_noint)
  
#multilevel linear regression with no covariates
  lmer_amyg_noise_baseline<-lmer(data= DROI_amyg, amyg_activation.noise_baseline~Parent_Alone_num+(1|Subject))
...
#examine relation with cortisol
```{r}
Dcort <- read.csv("data/cortisol_edited.csv")
DcortCRP <- Dcort[Dcort$Fraction2=="CRP", c("Subject","ug.dL")]
DcortnoCRP <- Dcort[Dcort$Fraction2=="noCRP",c("Subject","ug.dL")]
colnames(DcortCRP)[2] <- "ug.dL_CRP"
colnames(DcortnoCRP)[2] <- "ug.dL_noCRP"

DROI_amyg <- merge(DROI_amyg, DcortCRP, by="Subject",all.x=T, all.y=F)
DROI_amyg <- merge(DROI_amyg, DcortnoCRP, by="Subject",all.x=T, all.y=F)
DROI_mPFC <- merge(DROI_mPFC, DcortCRP, by="Subject",all.x=T, all.y=F)
DROI_mPFC <- merge(DROI_mPFC, DcortnoCRP, by="Subject",all.x=T, all.y=F)

lmer_cort <- lmer(data=DROI_amyg,amyg_activation.noise_baseline~ ug.dL_CRP + (1|Subject))
summary(lmer_cort)
lmer_cort <- lmer(data=DROI_amyg,amyg_activation.plus_minus~ ug.dL_CRP + (1|Subject))
summary(lmer_cort)

lmer_cort_mPFC <- lmer(data=DROI_mPFC,mPFC_activation.noise_baseline~ ug.dL_CRP + (1|Subject))
summary(lmer_cort_mPFC)
lmer_cort_mPFC <- lmer(data=DROI_mPFC,mPFC_activation.plus_minus~ ug.dL_CRP + (1|Subject))
summary(lmer_cort_mPFC)

lmer_cort <- lmer(data=DROI_amyg,amyg_activation.noise_baseline~ ug.dL_noCRP + (1|Subject))
summary(lmer_cort)
lmer_cort <- lmer(data=DROI_amyg,amyg_activation.plus_minus~ ug.dL_noCRP + (1|Subject))
summary(lmer_cort)

lmer_cort_mPFC <- lmer(data=DROI_mPFC,mPFC_activation.noise_baseline~ ug.dL_noCRP + (1|Subject))
summary(lmer_cort_mPFC)
lmer_cort_mPFC <- lmer(data=DROI_mPFC,mPFC_activation.plus_minus~ ug.dL_noCRP + (1|Subject))
summary(lmer_cort_mPFC)
```
